...
# this is old code - READ it and then modify as per the above plan .
...
# 2 bugs solve -
# if i select a object which is not there in the list (starlink) then it should send to ai
# and ai gives response , but thats not hapenning .
# add a new feature - selected object -> none . so when no object are selected then none hoga . since
# right now if i select a object and then select no object and press p then it tells about the last object selected .

# can work on better (text to speech) and (speech to text)
...
# celestial_info.txt ka star name or hip3434 - sterillium kay hisab say . one time checkout what
# the star name when fetch stellarium data is and what it its name in the celstial_info.txt .
        ...
        if current_object:
            ...
            if current_object in solar_system_objects:
                ...
                if ra and dec:
                    ...
                    alt, az = calculate_star_alt_az(
                        float(ra), float(dec), current_object
                    )
                else:
    ...
    if keyboard.is_pressed("q"):
        ...
        if query:
            ai_response = get_ai_response(
                f"{query} , tell me in 20 words without any symbols , bold ,*, etc "
            )
            if ai_response:
                print(ai_response)
